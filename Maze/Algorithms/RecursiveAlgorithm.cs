...
namespace Maze.Algorithms
{
	public class RecursiveAlgorithm : SolvingAlgorithmBase.SolvingAlgorithmBase
	{
		...
		{
			_visited = new bool[operation.Maze.Rows, operation.Maze.Columns];
			_correctPath = new bool[operation.Maze.Rows, operation.Maze.Columns];
		}

		public bool RecursiveSolve(int x, int y)
		{
			if (x == Operation.Maze.Exit.X && y == Operation.Maze.Exit.Y) return true;
			if (Operation.Maze.Map[x, y].Content == Content.Wall || _visited[x, y]) return false; // If you are on a wall or already were here
			_visited[x, y] = true;
